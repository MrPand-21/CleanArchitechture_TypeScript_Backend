...
export class CoreApiResponse<TData> {
    ...
    public static success<TData>(data?: TData, message?: string): CoreApiResponse<TData> {
        const resultCode: number = Result.SUCCESS.code;
        const resultMessage: string = message || Result.SUCCESS.message;
    ...
    public static error<TData>(code?: number, message?: string, data?: TData): CoreApiResponse<TData> {
        const resultCode: number = code || Result.INTERNAL_ERROR.code;
        const resultMessage: string = message || Result.INTERNAL_ERROR.message;
